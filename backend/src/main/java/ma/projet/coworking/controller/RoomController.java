...
public class RoomController {
    ...
    // Endpoint to add a new room, requires authentication and admin role
    @PostMapping("/add/new-room")
    @PreAuthorize("hasRole('ROLE_ADMIN')") // Only admins can add rooms
    public ResponseEntity<RoomResponse> addNewRoom(
        ...
        RoomResponse response = new RoomResponse(savedRoom.getId(), savedRoom.getRoomType(), savedRoom.getPricePerHour());
    ...
    // Endpoint to get all rooms, accessible by everyone
    ...
    // Endpoint to delete a room, requires admin role
    @DeleteMapping("/delete/room/{id}")
    @PreAuthorize("hasRole('ROLE_ADMIN')") // Only admins can delete rooms
    public ResponseEntity<Void> deleteRoom(@PathVariable Long id) {
        roomService.deleteRoom(id);
        return new ResponseEntity<>(HttpStatus.NO_CONTENT);
    }

    // Endpoint to update a room, requires admin role
    @PutMapping("/update/{id}")
    @PreAuthorize("hasRole('ROLE_ADMIN')") // Only admins can update rooms
    ...
    // Endpoint to get available rooms, accessible by authenticated users
    ...
    // Helper method to convert Room to RoomResponse
    private RoomResponse getRoomResponse(Room room) {
        List<BookedRoom> bookings = getAllBookingsByRoomId(room.getId());
        List<BookingResponse> bookingInfo = bookings.stream()
                .map(booking -> new BookingResponse(booking.getId(), booking.getDate(),
                        booking.getHeureDebut(), booking.getHeureFin(), booking.getBookingConfirmationCode()))
                .toList();
    ...
    // Helper method to get all bookings for a specific room
